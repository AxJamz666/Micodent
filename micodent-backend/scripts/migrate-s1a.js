...
async function run() {
  ...
  try {
    ...
    if (names.length !== 24 || names.some(n => n.startsWith('seguridad_')) || columns.some(c => c.COLUMN_NAME === 'auth_version')
    ...
    if (baseline.identity.db !== 'micodent_dev' || baseline.tables.length !== 24) throw new Error('BACKUP_BASELINE_REJECTED');
    ...
    console.log(JSON.stringify({ status: 'applied', migration: migration.id, checksum,
      identity: identity[0], originalTablesPreserved: 24, newTables: 4, existingPasswordsAndRolesUnchanged: true }));
  } catch (err) {
...
module.exports = { verifyApplied, checksum };
