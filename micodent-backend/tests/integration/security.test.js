...
const disposable = process.env.S1A_TEST_DATADIR && ['mysql8', 'mariadb'].includes(process.env.S1A_TEST_ENGINE)
  && /^\d+$/.test(process.env.DB_PORT || '') && !['3306', '3307', '3308'].includes(process.env.DB_PORT);
const originalTestInstance = process.env.DB_PORT === '3308' && process.env.TEST_SERVER_UUID;
if (process.env.S1A_TEST_MODE !== 'isolated' || (!disposable && !originalTestInstance)
    || process.env.DB_HOST !== '127.0.0.1' || process.env.DB_NAME !== 'micodent_dev'
...
test.before(async () => {
  const [[identity]] = await db.query('SELECT @@port AS port,@@datadir AS dir,DATABASE() AS db');
  assert.equal(Number(identity.port), Number(process.env.DB_PORT));
  if (disposable) assert.equal(path.resolve(identity.dir), path.resolve(process.env.S1A_TEST_DATADIR));
  if (process.env.TEST_SERVER_UUID) {
    const [[server]] = await db.query('SELECT @@server_uuid AS uuid');
    assert.equal(server.uuid, process.env.TEST_SERVER_UUID);
  }
...
test('two simultaneous personal password changes cannot both commit', async () => {
  ...
  assert.equal(results.filter(r => [401, 409].includes(r.status)).length, 1, JSON.stringify(results.map(r => r.status)));
  assert.equal((await request('GET', '/auth/me', token)).status, 401);
});

test('RC4 commission zero survives protected creation and editing without changing credentials', async () => {
  const token = await login(owner), id = prefix + 'commission';
  const data = { id, nombre: 'Prueba', nombre_completo: 'Cuenta sintetica comision',
    rol: 'Doctor', nivel: 1, password, comision_porcentaje: 0 };
  assert.equal((await request('POST', '/usuarios', token, data)).status, 201);
  const before = await credentials(id);
  const { password: _password, ...edit } = data;
  for (const rate of [25.5, 0]) {
    assert.equal((await request('PUT', '/usuarios/' + id, token, { ...edit, comision_porcentaje: rate })).status, 200);
    const [[row]] = await db.query('SELECT comision_porcentaje FROM usuarios WHERE id=?', [id]);
    assert.equal(Number(row.comision_porcentaje), rate);
    assert.deepEqual(await credentials(id), before);
  }
  for (const rate of [-1, 101, '1.001']) {
    assert.equal((await request('PUT', '/usuarios/' + id, token, { ...edit, comision_porcentaje: rate })).status, 400);
  }
  const [[row]] = await db.query('SELECT comision_porcentaje FROM usuarios WHERE id=?', [id]);
  assert.equal(Number(row.comision_porcentaje), 0);
});
