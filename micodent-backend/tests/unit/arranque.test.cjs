...
const launcher = require('../../../micodent-arranque.cjs');
...
test('lock: OS releases it when the owning process exits', async t => {
  const dir = fixture(t);
  const file = require.resolve('../../../micodent-arranque.cjs');
...
test('Windows: actual BAT propagates failure without exposing internals', {
  ...
  assert.match(result.stdout + result.stderr, /\[INSTALLATION\]/);
  assert.doesNotMatch(result.stdout + result.stderr, /at Object|at main|ENOENT|password/i);
});
