...
async function main() {
  ...
  children.push(cp.spawn(process.execPath, ['node_modules/vite/bin/vite.js', '--host', '127.0.0.1', '--port', '5173', '--strictPort'], { cwd: root + '/micodent-frontend', env: { ...process.env, VITE_API_BASE_URL: 'http://localhost:4401/api' }, stdio: 'ignore', windowsHide: true }));
  ...
  const oldCookie = (await page.context().cookies('http://localhost:4401/api')).find(c=>c.name==='micodent_dev_session_v2');
  ...
  assert.equal((await fetch('http://localhost:4401/api/auth/me', { headers: { Cookie: oldCookie.name+'='+oldCookie.value, 'X-Micodent-Client':'web', 'X-Micodent-Bootstrap':'1' } })).status, 401);
  ...
  const currentCookie = (await page.context().cookies('http://localhost:4401/api')).find(c=>c.name==='micodent_dev_session_v2');
  const currentMarker = await page.evaluate(() => localStorage.getItem('micodentSessionEvent'));
  ...
  assert.ok(await page.evaluate(marker => localStorage.getItem('micodentSessionEvent') === marker, currentMarker));
  ...
  assert.equal((await fetch('http://localhost:4401/api/auth/me', { headers: { Cookie: currentCookie.name+'='+currentCookie.value, 'X-Micodent-Client':'web', 'X-Micodent-Bootstrap':'1' } })).status, 401);
