...
El propietario solicita registrar y subir a GitHub cada etapa terminada, junto
con su informe de cierre. Durante la etapa se conservan cambios locales y se
evitan pushes intermedios salvo autorizacion expresa. Esta instruccion autoriza
versionar ese trabajo, NO iniciar automaticamente
...
3. Comprobar secretos conocidos, patrones y material privado en el commit y
   su historial antes de subir. Si esta disponible security:check y
   security:history, ambos deben finalizar correctamente. No usar
   --patterns-only para sustituir la comprobacion local de secretos conocidos.
...
5. Al cerrar la etapa, subir solo codigo y documentacion revisados a origin
   (AxJamz666/Micodent). No abrir PR sin autorizacion expresa del propietario;
   vincular issue y PR cuando efectivamente existan.
