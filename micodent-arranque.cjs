...
const MESSAGES = {
  ...
  MIGRATION: 'Falta completar la actualizacion financiera de esta base de datos. Contacte al administrador; no importe otra base ni reinstale MICODENT.',
...
async function healthy(config, timeout) {
  ...
  if (response?.status === 503 && response.type.includes('application/json')) {
    let body; try { body = JSON.parse(response.body); } catch {}
    if (body?.reason === 'schema_pending') fail('MIGRATION');
  }
...
async function boot(config, dependencies = {}) {
  ...
  op.notify('MYSQL_READY');
  ownership = await op.owner(config);
  if (ownership === 'foreign') fail('PORT_BUSY');
  let child;
  if (ownership === 'free') {
    op.log('BACKEND_START');
    op.notify('BACKEND_START');
    child = await op.start(config);
  } else {
    op.log('BACKEND_REUSE');
    op.notify('BACKEND_REUSE');
  }
  op.log('BACKEND_WAIT');
  op.notify('BACKEND_WAIT');
...
class ProgressUI {
  constructor(version) {
    ...
    this.current = 'files';
    this.message = 'Verificando archivos y configuracion...';
    this.supportLog = '';
    this.finalMessage = '';
    this.stages = {
      files: { state: 'waiting', text: 'Archivos y configuracion' },
      mysql: { state: 'pending', text: 'MySQL disponible' },
      backend: { state: 'pending', text: 'Conexion del sistema con la base de datos' },
      interface: { state: 'pending', text: 'Verificacion de la interfaz' },
      browser: { state: 'pending', text: 'Apertura del navegador' },
    };
  }
  setLog(file) { this.supportLog = file || ''; this.render(true); }
  set(stage, state, message) {
    if (this.stages[stage]) this.stages[stage].state = state;
    if (stage) this.current = stage;
    if (message) this.message = message;
    this.render(true);
  }
  handle(event) {
    if (event === 'MYSQL_WAIT') {
      this.set('mysql', 'waiting', 'Esperando que MySQL termine de iniciar. Puede tardar en equipos con pocos recursos.');
    } else if (event === 'MYSQL_READY') {
      this.stages.mysql.state = 'ok';
      this.set('backend', 'waiting', 'MySQL esta disponible. Preparando el servidor MICODENT...');
    } else if (event === 'BACKEND_START') {
      this.set('backend', 'waiting', 'Iniciando el servidor MICODENT de forma segura...');
    } else if (event === 'BACKEND_REUSE') {
      this.set('backend', 'waiting', 'Se detecto un servidor MICODENT existente. Verificando su estado...');
    } else if (event === 'BACKEND_WAIT') {
      this.set('backend', 'waiting', 'Esperando la conexion del sistema con la base de datos.');
    } else if (event === 'BACKEND_READY') {
      this.stages.backend.state = 'ok';
      this.set('interface', 'waiting', 'Servidor y base de datos listos. Verificando la interfaz...');
    } else if (event === 'BUILD_CHECK') {
      this.set('interface', 'waiting', 'Verificando que la interfaz cargue completa y corresponda a esta instalacion.');
    } else if (event === 'BUILD_READY') {
      this.stages.interface.state = 'ok';
      this.set('browser', 'pending', 'Sistema verificado. Preparando la apertura del navegador...');
    } else if (event === 'READY') {
      this.stages.mysql.state = 'ok';
      this.stages.backend.state = 'ok';
      this.stages.interface.state = 'ok';
      this.set('browser', 'waiting', 'MICODENT esta listo. Abriendo el navegador...');
    }
  }
  browserReady() {
    this.stages.browser.state = 'ok';
    this.finalMessage = 'MICODENT ESTA LISTO';
    this.message = 'El sistema fue verificado y el navegador fue abierto.';
    this.render(true);
  }
  browserFailed(url) {
    this.stages.browser.state = 'error';
    this.finalMessage = 'MICODENT ESTA LISTO, PERO EL NAVEGADOR NO SE ABRIO';
    this.message = `Abra manualmente: ${url}`;
    this.render(true);
  }
  error(code, message) {
    if (this.stages[this.current]) this.stages[this.current].state = 'error';
    this.finalMessage = 'MICODENT NO PUDO INICIARSE';
  ...
  render(force = false) {
    ...
    lines.push('============================================================');
    lines.push('                         MICODENT');
    lines.push(`                    Inicio verificado V${this.version}`);
    lines.push('============================================================');
    lines.push('');
    for (const key of ['files', 'mysql', 'backend', 'interface', 'browser']) {
      const item = this.stages[key];
      lines.push(`${stageLabel(item.state).padEnd(12)} ${item.text}`);
    }
    lines.push('');
    lines.push(`Tiempo transcurrido: ${elapsedText(this.startedAt)}`);
    lines.push(`Estado: ${this.message}`);
    lines.push('');
    if (this.finalMessage) {
      lines.push('------------------------------------------------------------');
      lines.push(`  ${this.finalMessage}`);
      lines.push('------------------------------------------------------------');
      lines.push('');
    } else {
      lines.push('El inicio continua automaticamente. No cierre esta ventana.');
      lines.push('');
    }
    if (this.supportLog) lines.push(`Registro de soporte: ${this.supportLog}`);
    process.stdout.write(lines.join('\n') + '\n');
  }
}

function checkModeOutput(config) {
  console.log(`MICODENT - Comprobacion V${VERSION}`);
  console.log('');
  console.log('[OK] Archivos y configuracion compatibles.');
  console.log(`[OK] Node ${process.versions.node}.`);
  console.log(`[OK] Puerto backend: ${config.backendPort}. Puerto MySQL: ${config.dbPort}.`);
  console.log(`[OK] Recursos del frontend verificados: ${config.resources.length}.`);
  console.log('');
  console.log('SOLO LECTURA: esta comprobacion no inicia procesos, no consulta MySQL');
  console.log('ni modifica la base de datos, .env, uploads o archivos clinicos.');
}

async function main() {
  let release; let log = () => {}; let timer; let ui;
  try {
    ...
    if (process.argv[2] === '--comprobar') {
      ...
      checkModeOutput(config);
      return;
    }

    ui = new ProgressUI(VERSION);
    ui.render(true);
    timer = setInterval(() => ui.render(false), 1000);

    const config = inspect(__dirname);
    ui.stages.files.state = 'ok';
    ui.set('mysql', 'pending', 'Archivos y configuracion compatibles. Preparando el inicio...');

    release = await lock(config.backend);
    if (!release) {
      ui.stages.files.state = 'ok';
      ui.finalMessage = 'YA HAY UN INICIO EN CURSO';
      ui.message = 'Espere a que termine la primera ventana de MICODENT.';
      ui.render(true);
      await sleep(1800);
      return;
    }
    ...
    const logFile = path.join(folder, 'arranque.log');
    log = logger(folder); log('BEGIN');
    ui.setLog(logFile);

    await boot(config, {
      log,
      notify: event => ui.handle(event),
    });
    ...
    if (ui) {
      ui.error(code, MESSAGES[code]);
      console.error('');
      console.error('No se borraron datos ni se detuvo ningun servicio.');
      console.error('No reinstale ni restaure la base de datos.');
    } else {
